...
class SettingsListTeacher extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Stream<int> unreadMessagesStream() async* {
      await for (var chatsSnap in FirebaseFirestore.instance.collection('chats').snapshots()) {
        int total = 0;
          ...
          child: Column(
            children: [
              SettingsCard(
                ...
                title: 'الرسائل',
                subtitle: 'لديك ${unreadCount} رسالة جديدة',
                badgeCount: unreadCount,
                onTap: () async {
                  final chatsSnap = await FirebaseFirestore.instance.collection('chats').get();
                  for (var chatDoc in chatsSnap.docs) {
                    final messagesSnap = await chatDoc.reference.collection('messages').get();
                    for (var msgDoc in messagesSnap.docs) {
                      final data = msgDoc.data() as Map<String, dynamic>;
